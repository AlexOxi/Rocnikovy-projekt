<template>
  <div class="non-selectable">
    ...
    <q-list v-for="(question, idx) in questions" :key="question.id" :class="question.answered ? 'bg-green-2' : ''">
      ...
      <q-item tag="label" v-for="(choice, idx) in question.choices" :key="choice.id * 1000 + choice.id"
        :disable="isShowResult">
        <q-item-section side class="text-bold">{{ idx + 1 }}.</q-item-section>
        <q-item-section side top>
          <q-checkbox v-model="selectedChoices" :val="choice.id" :disable="isShowResult" />
        </q-item-section>
...
import { getTest, clearAllAnswerFlags, getQuestionFromChoiceId } from '../data/data-layer'
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      selectedChoices.value = [1003, 2002]
      checkAnswers()
      checkTest()
      // TODO: REMOVE AFTER TESTING
    })
